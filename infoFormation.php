...
<div class="formulaire pied_page">
	...
	if ($name && $firstname && $mail) {
		...
		$choix=$nChoix;
		$txtBtn = "Corriger";
		$sqlins = "INSERT INTO users (name, firstname, mail,choix_1) VALUES ('$name', '$firstname','$mail','$choix') ON DUPLICATE KEY UPDATE name='$name',firstname='$firstname',choix_1=$choix";
		if ($connexion -> query($sqlins)) {
			echo "<p>Requete Ins ok</p>".$choix;
			//echo "<div id='dialog' title='Confirmation'><p>Merci pour votre participation</p></div>";
		} else {
			echo "<p>Requete Ins planté</p>";
		}
	}
	//$connexion -> close();
    ?>
...
//$connexion = new mysqli(HOST, USER, PWD, DB);
	...
	foreach ($cate as $c){
		...
		$leveln="niveau_".$i;		
		$sqlu=("UPDATE users SET $choixn=$id, $priorityn=$Pos, $leveln=$level WHERE mail='$mail'");
